...
def truck_affectation(G,list_route,list_trucks):
    ...
    for route in list_route: #For each route, we will identify the cheapest truck to do it
        ...
        path,power_min = new_minpower_aux(g_mst, src, dest)
...
def knapsack_cost(G,list_trucks, list_route, my_B): #We want to have the highest sum of power with our trucks
    ...
    list_trucks_affected.sort(key=lambda l : l[0][1], reverse=True) #We sort the affectation by descending truck costs 
    selected_itineraries = [] #only the routes with the highest efficency will be selected
    not_selected_itineraries = [] #We also want to know which routes are not done to compare with knapsack_efficency
    list_profit = []
    for index_itinerary in range(len(list_trucks_affected)):
        ...
        if cost + total_cost < my_B: #We add a route by descending efficency and only if it is in our budget : maybe there is further a route with a lower profit but which need a truck which is still in our budget
            ...
            selected_itineraries.append(None) #We have a None to say that a route has not been taken
            not_selected_itineraries.append(association) #The routes which have not been taken (whose the length = number on None in the other list)
